...
Although in the paper they analyze 33 RNASeq datasets from TCGA, only there `GSEABenchmarkeR::loadEData("tcga")` only returns 15 data sets. 
We follow a vanilla DE pipeline outlined in the `GSEABenchmarkR` documentation, and then apply a suite of gene list based GSEA methods. Finally we use the MalaCards derived relevance scores to evaluate results.
...
tar_load('gseabenchmark.size_lasso')
tar_load('gseabenchmark.size_mr.ash.lasso')

tar_load('gseabenchmark.size_susie.veb.boost')
tar_load('gseabenchmark.size_susie.veb.boost32')
tar_load('gseabenchmark.size_susie.veb.boost64')

tar_load('gseabenchmark.size_logistic.susie.veb.boost')
tar_load('gseabenchmark.size_logistic.susie.veb.boost32')
tar_load('gseabenchmark.size_logistic.susie.veb.boost64')

tar_load('gseabenchmark.size_lasso')
tar_load('gseabenchmark.size_elastic.net')
tar_load('gseabenchmark.size_fet')

result <- bind_rows(
  gseabenchmark.size_fet,
  gseabenchmark.size_mr.ash.lasso,
  gseabenchmark.size_lasso,
  gseabenchmark.size_susie.veb.boost,
  gseabenchmark.size_susie.veb.boost32,
  gseabenchmark.size_susie.veb.boost64,

  gseabenchmark.size_logistic.susie.veb.boost,
  gseabenchmark.size_logistic.susie.veb.boost32,
  gseabenchmark.size_logistic.susie.veb.boost64,

  gseabenchmark.size_lasso,
  gseabenchmark.size_elastic.net
)

result %>% unnest(c(positives, thresh)) %>% filter(method == 'logistic.susie.veb.boost', near(thresh, 0.95))

result %>% unnest(c(positives, thresh)) %>%
  filter(near(thresh, 0.95)) %>%
  filter(grepl('susie', method)) %>%
  mutate(positives = ifelse(positives > 20, 20, positives)) %>%
  ggplot(aes(x=positives)) + geom_bar() + facet_wrap(vars(method))
...
asymmetric <- apply(overlap, 1, function(x) x / gs.size)
```
