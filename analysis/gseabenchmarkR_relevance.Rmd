...
```{r}
logistic.susie.pip %>% rowwise() %>%
  mutate(
    pip.sets = list(names(which(pip > 0.95))),
    relevant.sets = list(intersect(colnames(jaccard), rownames(gobp.relevance[[ID]]))),
    max.prop.relevant.in.set = list(apply(as.matrix(jaccard[pip.sets, relevant.sets]), 1, max))
  ) %>% select(ID, max.prop.relevant.in.set) %>% unnest(max.prop.relevant.in.set) %>%
  ggplot(aes(x=max.prop.relevant.in.set)) + geom_histogram()
```

Most of the gene sets with high PIPs do not have much to do with the relevant gene sets. The histgram shows $\max_i\frac{|S \cap  R_i|}{|S \cup R_i|}$ where $S$ is the gene set with large pip and $\{R_i\}$ are the set of relevant gene sets. We would have liked to see that the 
