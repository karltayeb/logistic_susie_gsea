...
```{r,}
result <- tar_agg('gseabenchmark.size', dplyr::starts_with)

result %>% unnest(c(positives, thresh)) %>%
  filter(near(thresh, 0.95)) %>%
  group_by(ID, method, n_genes) %>% summarise(n.hits = sum(positives)) %>%
  ggplot(aes(x=method, y=n.hits, fill=method)) + geom_col() + facet_wrap(vars(ID))
```
...
library(ggvenn)
x <- list(
  FET = unique(unlist(fet.result$pip.sets)), 
  ElasticNet = unique(unlist(en.result$pip.sets)), 
  LogisticSuSiE = unique(unlist(logistic.susie.pip$pip.sets))
)
ggvenn(
  x, 
...
x <- list(
  ...
  ElasticNet = unique(unlist(en.result$pip.sets)), 
  LogisticSuSiE = unique(unlist(logistic.susie.pip$pip.sets)),
  Relevant = unique(unlist(fet.result$pip.sets))
)
ggvenn(
  x, 
