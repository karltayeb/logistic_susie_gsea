...
The simulation here are simple: There will be a set of `L` *active* gene sets whose genes are observed with log odds controlled by parameter `active.logit`. All other genes will be observed with log odds controlled by parameter `background.logit`. $X$ is an $n \times p$ binary matrix indicating membership of genes in gene clusters For $n$ genes and $p$ gene sets.
...
interestGene = referenceGene[sim$Y[, 1] == 1]
f = length(interestGene) / length(referenceGene)
ORA <- geneSet %>% 
  group_by(geneSet) %>% 
  summarise(
...
ORA <- ORA %>%
  mutate(
    active = (geneSet %in% geneSets[sim$active]),
    pValue=1-phyper(overlap - 1, n.interestGene, n.referenceGene - n.interestGene, size, lower.tail=TRUE, log.p=FALSE),
    FDR = p.adjust(pValue, method = 'fdr')
  ) %>%
...
X_mat_trunc <- X_mat[, (colSums(X_mat) > 10 & colSums(X_mat) < 5000)]
hist(colSums(X_mat_trunc), main = 'Gene Set Sizes')
hist(rowSums(X_mat_trunc), main='Gene Sets per Gene')
...
First we do a simulation with no active gene sets. We set the background probability of a gene being included in the gene list at around $0.05$. We compare how a hypergeometric test behave.
...
P values are just slightly deflated for hypergeometric p values. I think you would expect this due to overlap, since the tests are not independent. Although this is quite different from what you might see in real data where genes are often correlated. In this case you would expect the test to be anti-conservative.
...
SuSiE tends to give smaller gene sets slightly larger PIP. This makes sense, it is more likely that a small gene set gets a higher proportion of genes than background by chance. Not a problem with Fisher. Hypergeometric distribution with fewer samples (corresponding with a smaller gene set) have higher variance, and the pvalues reflect that.
...
Good to see that in all the simulations we did not get any cases where logistic SuSiE returns small credible sets.
...
Next we simulate one active gene set. Genes in the active gene set will be included in the gene list `active.logit = -2` with probability $p = 0.12$. Since the gene set is small relative to the background gene set, most of the genes included in the list are junk.
...
susie.result5 %>% compute.cset.sizes(., alpha=0.99) %>% filter(cset_size < 100) %>% ggplot(aes(x=cset_size)) + geom_histogram()
```
...
Next lets vary the frequency of background genes that make it into the list.
...
### Increasing overlap, $\pi_0 \approxi 0.25$, $\pi = 1$
...
### Increasing overlap, $\pi_0 \approxi 0.12$, $\pi aprox 0.73$
